...
for i in range(1, 11):
    with st.sidebar.container():
        ...
        cols = st.columns(3)
        ec = cols[0].number_input(f"EC", min_value=0.0, max_value=150.0, value=75.0, key=f"ec_{i}")
        temp = cols[1].number_input(f"Temp", min_value=0.0, max_value=50.0, value=22.0, key=f"temp_{i}")
        occ = cols[2].number_input(f"Occ", min_value=0.0, max_value=100.0, value=50.0, key=f"occ_{i}")
        ec_lags.append(ec)
